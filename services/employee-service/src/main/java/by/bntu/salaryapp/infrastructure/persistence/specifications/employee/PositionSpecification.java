...
public record PositionSpecification(PositionFilterDto filter) implements Specification<Position> {
    @Override
    public Predicate toPredicate(@NonNull Root<Position> root,
                                 CriteriaQuery<?> query,
        ...
        if (filter.getUpdatedBy() != null) {
            Join<Position, User> join = root.join("updatedBy", JoinType.INNER);
            predicates.add(criteriaBuilder.equal(join.get("updatedBy"), filter.getUpdatedBy()));
        }
        if (filter.getCreatedBy() != null) {
            Join<Position, User> join = root.join("createdBy", JoinType.INNER);
